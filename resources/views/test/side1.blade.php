...
<button type="button" x-on:click="showSidebar = !showSidebar" class="" aria-label="open sidebar">asdfasdfasdf</button>
   ...
   x-cloak
    x-show="showSidebar"
    x-trap="showSidebar"
    class="fixed right-0 z-20 flex h-svh w-80 shrink-0 flex-col border-l border-outline bg-surface-alt p-4 transition-transform duration-300 dark:border-outline-dark dark:bg-surface-dark-alt" aria-label="shopping cart" x-transition:enter="transition duration-200 ease-out" x-transition:enter-end="translate-x-0" x-transition:enter-start=" translate-x-80" x-transition:leave="transition ease-in duration-200 " x-transition:leave-end="translate-x-80" x-transition:leave-start="translate-x-0">
